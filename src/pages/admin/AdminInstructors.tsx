      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto border-border bg-background sm:max-w-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 pt-1">
          ...
          <DialogFooter className="pt-1">
            <button type="button" onClick={() => onOpenChange(false)} className="rounded-full btn-press border border-border px-4 py-2 text-xs font-medium text-muted-foreground">
              Cancelar
            </button>
    ...
    <Dialog open onOpenChange={(v) => { if (!v) onCancel(); }}>
      <DialogContent className="border-border bg-background sm:max-w-xs">
        ...
        <DialogFooter className="pt-2">
          <button onClick={onCancel} className="rounded-full btn-press border border-border px-4 py-2 text-xs font-medium text-muted-foreground">Cancelar</button>
          <button onClick={onConfirm} className="rounded-full btn-press bg-red-500 px-4 py-2 text-xs font-medium text-white">Remover</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
    ...
    <AdminLayout title="Instrutores">
      ...
      {!isLoading && instructors.length === 0 && (
        <div className="mt-16 flex flex-col items-center gap-3 text-center">
          ...
          <button onClick={() => setShowCreate(true)} className="rounded-full btn-press bg-primary px-4 py-2 text-xs font-medium text-primary-foreground">
