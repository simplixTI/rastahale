      ...
      {addModal && (
        <div className="mt-2 flex items-center gap-2">
          ...
          <button onClick={handleAddModality} disabled={createModality.isPending}
            className="flex shrink-0 items-center gap-1 rounded-full btn-press bg-primary px-3 py-2 text-xs font-medium text-primary-foreground disabled:opacity-50">
        ...
        <Dialog open onOpenChange={(v) => { if (!v) setDelModule(null); }}>
          <DialogContent className="border-border bg-background sm:max-w-xs">
            ...
            <DialogFooter className="pt-2">
              <button onClick={() => setDelModule(null)} className="rounded-full btn-press border border-border px-4 py-2 text-xs font-medium text-muted-foreground">Cancelar</button>
              <button onClick={handleDeleteModule} disabled={deleteModule.isPending} className="rounded-full btn-press bg-red-500 px-4 py-2 text-xs font-medium text-white disabled:opacity-50">Remover</button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
      ...
      {delModal && (
        <Dialog open onOpenChange={(v) => { if (!v) setDelModal(null); }}>
          <DialogContent className="border-border bg-background sm:max-w-xs">
            ...
            <DialogFooter className="pt-2">
              <button onClick={() => setDelModal(null)} className="rounded-full btn-press border border-border px-4 py-2 text-xs font-medium text-muted-foreground">Cancelar</button>
              <button onClick={handleDeleteModality} disabled={deleteModality.isPending} className="rounded-full btn-press bg-red-500 px-4 py-2 text-xs font-medium text-white disabled:opacity-50">Remover</button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
