    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto border-border bg-background sm:max-w-md">
        ...
        <DialogFooter className="pt-2">
          <button type="button" onClick={() => onOpenChange(false)} className="rounded-full btn-press border border-border px-4 py-2 text-xs font-medium text-muted-foreground">Cancelar</button>
          <button onClick={handleSave} className="rounded-full btn-press bg-primary px-4 py-2 text-xs font-medium text-primary-foreground">
      ...
      {delSess && (
        <Dialog open onOpenChange={(v) => { if (!v) setDelSess(null); }}>
          <DialogContent className="border-border bg-background sm:max-w-xs">
            ...
            <DialogFooter className="pt-2">
              <button onClick={() => setDelSess(null)} className="rounded-full btn-press border border-border px-4 py-2 text-xs font-medium text-muted-foreground">Cancelar</button>
              <button onClick={() => { remove(delSess.id); toast.success("Sessão removida", { duration: 1500 }); setDelSess(null); }} className="rounded-full btn-press bg-red-500 px-4 py-2 text-xs font-medium text-white">Remover</button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
